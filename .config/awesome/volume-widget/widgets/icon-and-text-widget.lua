	...
	return wibox.widget({
		...
		set_volume_level = function(self, new_value)
			...
			if self.is_muted then
				volume_icon_name = "mute24.png"
			else
				local new_value_num = tonumber(new_value)
				if new_value_num >= 0 and new_value_num < 33 then
					volume_icon_name = "audio_low_panel_volume_icon.png"
				elseif new_value_num < 66 then
					volume_icon_name = "audio_medium_panel_volume_icon.png"
				else
					volume_icon_name = "audio_high_panel_volume_icon.png"
				end
			end
