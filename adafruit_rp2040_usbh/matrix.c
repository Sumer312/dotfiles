...
static bool send_led_report(uint8_t* leds) {
    if (kbd_addr != 0) {
        bool res = tuh_hid_set_report(kbd_addr, kbd_instance, 0, HID_REPORT_TYPE_OUTPUT, leds, sizeof(*leds));
        uprintf("LED report sent: 0x%02X, result: %d\n", *leds, result);
        return res;
    }
    uprintf("LED report skipped: kbd_addr is 0\n");
    return false;
}
